...
export const PUT = handler(async (request: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  if (!(await integrationsRepo.setEnabled("incoming", id, workspaceId, isEnabled))) {
    return problem(404, "not_found", "Not found");
  }
  await audit({
    actorId: user.id,
    action: isEnabled ? "integration.incoming_webhook_enabled" : "integration.incoming_webhook_disabled",
    category: "channel",
    severity: "warning",
    target: id,
    request,
